...
// 일정 저장
async function saveEvent(date, title, category, eventId = null, year = null, month = null, day = null) {
  console.log('💾 일정 저장 시작:', { date, title, category, eventId, year, month, day });

  try {
    const connected = await testConnection();
    if (connected) {
      return await saveToDatabase(date, title, category, eventId, year, month, day);
    } else {
      console.log('📱 연결 실패 - localStorage 사용');
      return await saveToLocalStorage(date, title, category, eventId, year, month, day);
    }
  } catch (error) {
...
// 데이터베이스에 저장
async function saveToDatabase(date, title, category, eventId = null, year = null, month = null, day = null) {
  const method = eventId ? 'PUT' : 'POST';
  const body = eventId
    ? { id: eventId, title, category, year, month, day }
    : { date_value: date, title, category, year, month, day };
...
// localStorage에 저장
async function saveToLocalStorage(date, title, category, eventId = null, year = null, month = null, day = null) {
  const events = JSON.parse(localStorage.getItem('calendarEvents') || '[]');

  if (eventId) {
    const index = events.findIndex(e => e.id == eventId);
    if (index !== -1) {
      events[index] = { ...events[index], title, category, year, month, day };
    }
  } else {
    events.push({
      id: Date.now(),
  ...
  return events[events.length - 1] || { id: Date.now(), date_value: date, title, category, year, month, day };
}
